...
public class ImageUndistortion{
	...
	public void doImageUndstortion(){
		...
		int caseNo = 0;
		String filename = "/proj/i5dmip/ym14utah/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame32.jpg";
		
		if(caseNo == 0)
		{
			filename = "/proj/i5dmip/ym14utah/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame32.jpg";
		}
		else if(caseNo == 1)
		{
			filename = "/proj/i5dmip/ym14utah/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/undistorted.jpg";
		}else if(caseNo == 2)
		{
			filename = "/proj/i5dmip/ym14utah/Reconstruction/CONRAD/src/edu/stanford/rsl/tutorial/dmip/frame90.jpg";
		}
		...
		for(int x = 0; x < imSize; x++)
		{
			for(int y = 0; y < imSize; y++)
			{
				...
				for(int k = 0; k <= degree; k++)
				{
					...
					{
						xDist.setAtIndex(x, y, (float)(
								xDist.getAtIndex(x, y)
								+ u_vec.getElement(cc)
								*Math.pow(x, k) * Math.pow(y,l) ) );
						yDist.setAtIndex(x, y, (float)(
								yDist.getAtIndex(x, y)
								+ v_vec.getElement(cc)
								*Math.pow(x, k) * Math.pow(y,l) ) );						
						cc++;
					}
		...
		for(int i = 0; i < imSize; i++)
		{
			...
			{
				float val = InterpolationOperators.interpolateLinear(distortedImage,
						xDist.getAtIndex(i, j), yDist.getAtIndex(i, j));
